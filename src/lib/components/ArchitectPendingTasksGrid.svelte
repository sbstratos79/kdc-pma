<script lang="ts">
	...
	let todaysPendingArchitectTasks = $derived(
		$architectData.flatMap((architect) =>
			architect.tasks.filter((task) => {
				if (!task.taskDueDate) {
					return false;
				}
...
{:else if todaysPendingArchitectTasks.length === 0}
...
{:else}
	<div class="space-y-4" style="column-width: 400px; column-gap: 1rem;">
		{#each todaysPendingArchitectTasks as task (task.taskId)}
			{#if task}
				<div
