...
export async function makeArchitect(overrides: Partial<typeof architects.$inferInsert> = {}) {
	const id = uid();
...
export async function makeProject(overrides: Partial<typeof projects.$inferInsert> = {}) {
	const id = uid();
	const rows = await testDb
		.insert(projects)
