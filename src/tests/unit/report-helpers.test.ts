...
describe('inDateRange (via getProjectStatusSummary)', () => {
  ...
  it('includes a record whose addedTime equals dateFrom (inclusive boundary)', async () => {
    await makeProject({ addedTime: TODAY });
    expect(await getProjectStatusSummary({ dateFrom: TODAY })).toHaveLength(1);
  });

  it('includes a record whose addedTime equals dateTo (inclusive boundary)', async () => {
    await makeProject({ addedTime: TODAY });
    expect(await getProjectStatusSummary({ dateTo: TODAY })).toHaveLength(1);
  });

  it('excludes a record whose addedTime is before dateFrom', async () => {
    await makeProject({ addedTime: YESTERDAY });
    expect(await getProjectStatusSummary({ dateFrom: TODAY })).toHaveLength(0);
  });

  it('excludes a record whose addedTime is after dateTo', async () => {
    await makeProject({ addedTime: TODAY });
  ...
  it('excludes records outside a narrow date range and keeps those inside', async () => {
    await makeProject({ addedTime: LAST_WEEK });  // outside
    await makeProject({ addedTime: TODAY });       // inside
    const result = await getProjectStatusSummary({ dateFrom: YESTERDAY, dateTo: TOMORROW });
    expect(result).toHaveLength(1);
  });

  it('filters tasks by addedTime for workload queries', async () => {
    const proj = await makeProject();
    await makeTask(proj.id, { addedTime: TODAY });
    await makeTask(proj.id, { addedTime: LAST_WEEK });
