...
describe('getProjectStatusSummary', () => {
	...
	it('filters by dateFrom/dateTo on dueDate', async () => {
		await makeProject({ dueDate: LAST_WEEK });
	...
	it('filters projects by search on project name', async () => {
		await makeProject({ name: 'Alpha Tower' });
		await makeProject({ name: 'Beta Bridge' });
		const result = await getProjectStatusSummary({ search: 'Alpha' });
		expect(result).toHaveLength(1);
		expect(result[0].projectName).toBe('Alpha Tower');
	});

	it('filters projects by search on project description', async () => {
		await makeProject({ name: 'A', description: 'Special building' });
		await makeProject({ name: 'B', description: 'Ordinary shed' });
		const result = await getProjectStatusSummary({ search: 'Special' });
		expect(result).toHaveLength(1);
	});
});
...
describe('getArchitectWorkload', () => {
	...
	it('filters tasks by date range', async () => {
		const arch = await makeArchitect();
		const proj = await makeProject();
		await makeTask(proj.id, { architectId: arch.id, dueDate: LAST_WEEK });
		await makeTask(proj.id, { architectId: arch.id, dueDate: TODAY });
	...
	it('filters tasks by status', async () => {
		...
		await makeTask(proj.id, { architectId: arch.id, status: 'Planning' });
		await makeTask(proj.id, { architectId: arch.id, status: 'Completed' });
		const [row] = await getArchitectWorkload({ status: 'In Progress' });
		expect(row.totalTasks).toBe(1);
	});
	...
	it('filters tasks by priority', async () => {
		...
		await makeTask(proj.id, { architectId: arch.id, priority: 'High' });
		await makeTask(proj.id, { architectId: arch.id, priority: 'Low' });
		const [row] = await getArchitectWorkload({ priority: 'High' });
		expect(row.totalTasks).toBe(1);
	});
});
...
describe('getOverdueTasks', () => {
	...
	it('filters by date range on dueDate', async () => {
		const proj = await makeProject();
		await makeTask(proj.id, { dueDate: LAST_WEEK, status: 'In Progress' });
		await makeTask(proj.id, { dueDate: YESTERDAY, status: 'In Progress' });
	...
	it('filters by search on task name', async () => {
		const proj = await makeProject();
		await makeTask(proj.id, { name: 'Fix Foundation', dueDate: YESTERDAY, status: 'Planning' });
		await makeTask(proj.id, { name: 'Paint Walls', dueDate: YESTERDAY, status: 'Planning' });
		const result = await getOverdueTasks({ search: 'Fix' });
		expect(result).toHaveLength(1);
		expect(result[0].taskName).toBe('Fix Foundation');
	});
	...
	it('filters by status', async () => {
		...
		await makeTask(proj.id, { dueDate: YESTERDAY, status: 'In Progress' });
		await makeTask(proj.id, { dueDate: YESTERDAY, status: 'Planning' });
		const result = await getOverdueTasks({ status: 'In Progress' });
		expect(result).toHaveLength(1);
...
describe('edge cases', () => {
	...
	it('search filter propagates to all sub-reports', async () => {
		const proj = await makeProject({ name: 'Unique Searchable Project' });
		await makeTask(proj.id, {
			name: 'Unique Searchable Task',
			dueDate: YESTERDAY,
			status: 'Planning'
		});
		await makeProject({ name: 'Other Project' });

		const report = await getAllReports({ search: 'Unique' });
		expect(report.projectSummary).toHaveLength(1);
		expect(report.projectSummary[0].projectName).toBe('Unique Searchable Project');
	});
});
