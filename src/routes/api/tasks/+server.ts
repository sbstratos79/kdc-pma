...
// Map repo DTO (already shaped by getTaskWithSubtasks) passthrough
function ensureTaskDto(t: any): Task {
  // Minimal normalization — your getTaskWithSubtasks returns correct shape already
  return {
    ...
    subtasks: (t.subtasks ?? []).map((s: any) => ({
      subtaskId: s.subtaskId ?? s.subtask_id ?? '',
      subtaskName: s.subtaskName ?? s.name ?? '',
      subtaskDescription: s.subtaskDescription ?? s.description ?? null,
      subtaskStatus: s.subtaskStatus ?? s.status ?? '',
      taskId: s.taskId ?? s.task_id ?? t.taskId ?? '',
      taskName: t.taskName ?? ''
    }))
  };
}
...
export const GET: RequestHandler = async () => {
  try {
    // repoListTasks returns raw rows (not full DTO), so we will call getTaskWithSubtasks for each
    ...
    const tasks: Task[] = [];
    for (const r of raw) {
      const dto = await getTaskWithSubtasks(r.taskId);
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    // Return full DTO including subtasks
    const dto = await getTaskWithSubtasks(created.taskId);
...
export const PUT: RequestHandler = async ({ request, params, url }) => {
  ...
  try {
    ...
    const dto = await getTaskWithSubtasks(updated.taskId);
